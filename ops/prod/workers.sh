COMMAND=$1

if [[ "$COMMAND" == "start" ]]; then
  sudo systemctl ${COMMAND} parallel-email-events-queue.service
  sudo systemctl ${COMMAND} parallel-email-sender-queue.service
  sudo systemctl ${COMMAND} parallel-event-processor-queue.service
  sudo systemctl ${COMMAND} parallel-signature-worker-queue.service
  sudo systemctl ${COMMAND} parallel-task-worker-queue.service
  sudo systemctl ${COMMAND} parallel-delay-queue.service
  sudo systemctl ${COMMAND} parallel-webhooks-worker-queue.service
  sudo systemctl ${COMMAND} parallel-background-check-profile-search-queue.service
  sudo systemctl ${COMMAND} parallel-low-priority-event-queue.service

  if [[ "#INSTANCE_NUMBER#" == "1" ]]; then
    sudo systemctl ${COMMAND} parallel-reminder-trigger-cron.service
    sudo systemctl ${COMMAND} parallel-scheduled-trigger-cron.service
    sudo systemctl ${COMMAND} parallel-petition-notifications-cron.service
    sudo systemctl ${COMMAND} parallel-organization-limits-cron.service
    sudo systemctl ${COMMAND} parallel-anonymizer-cron.service
    sudo systemctl ${COMMAND} parallel-old-notifications-cron.service
    sudo systemctl ${COMMAND} parallel-expiring-properties-cron.service
    sudo systemctl ${COMMAND} parallel-background-check-monitor-cron.service
    sudo systemctl ${COMMAND} parallel-adverse-media-monitor-cron.service
    sudo systemctl ${COMMAND} parallel-sap-profile-polling-cron.service
  fi 
else
  sudo systemctl ${COMMAND} parallel-*-queue.service
  if [[ "#INSTANCE_NUMBER#" == "1" ]]; then
    sudo systemctl ${COMMAND} parallel-*-cron.service
  fi
fi
