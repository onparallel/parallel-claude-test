...
export async function up(knex: Knex): Promise<void> {
  await knex.raw(/* sql */ `
    alter type "petition_signature_status" add value 'CANCELLING' after 'PROCESSED';
    commit;
    ...
    check (
      (status in ('CANCELLING', 'CANCELLED') and cancel_reason is not null and cancel_data is not null) 
      or 
      (status not in ('CANCELLING', 'CANCELLED') and cancel_reason is null and cancel_data is null)
    )
  `);
}
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.raw(/* sql */ `
    update petition_signature_request set "status" = 'CANCELLED' where "status" = 'CANCELLING';
  `);

  await knex.raw(/* sql */ `
    alter type petition_signature_status rename to petition_signature_status_old;
    create type petition_signature_status as enum ('ENQUEUED', 'PROCESSING', 'PROCESSED', 'CANCELLED', 'COMPLETED');
    alter table petition_signature_request 
