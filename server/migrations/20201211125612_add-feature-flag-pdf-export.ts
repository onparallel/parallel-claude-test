...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.raw(/* sql */ `
    start transaction;
    alter type feature_flag_name add value 'PETITION_PDF_EXPORT';
    commit;
`);
...
export async function down(knex: Knex): Promise<void> {
  await knex
    ...
    .where("feature_flag_name", "PETITION_PDF_EXPORT")
    .delete();
  await knex.from("feature_flag").where("name", "PETITION_PDF_EXPORT").delete();
  ...
  // recreate feature_flag_name enum without PETITION_PDF_EXPORT
  await knex.raw(/* sql */ `
    alter type feature_flag_name rename to feature_flag_name_old;
    create type feature_flag_name as enum ('PETITION_SIGNATURE', 'INTERNAL_COMMENTS');
