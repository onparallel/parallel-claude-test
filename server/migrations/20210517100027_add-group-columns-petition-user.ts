...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.raw(/* sql */ `
    ...
    on "petition_user" (user_id, petition_id) where deleted_at is null and from_user_group_id is null;
    
    -- unique group assigned users per petition
    create unique index "petition_user__from_user_group_id__user_id__petition_id" 
    on "petition_user" (from_user_group_id, user_id, petition_id) where deleted_at is null;
    
    -- unique groups per petition
    create unique index "petition_user__user_group_id__petition_id" 
    on "petition_user" (user_group_id, petition_id) where deleted_at is null;
