...
export async function down(knex: Knex): Promise<void> {
  await knex.raw(/* sql */ `
    ...
    CREATE INDEX pun__unprocessed_comment_notifications ON public.petition_user_notification USING btree (id) WHERE ((type = 'COMMENT_CREATED'::petition_user_notification_type) AND (is_read = false) AND (processed_at IS NULL));
`);
}
