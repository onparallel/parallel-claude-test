...
scim
  ...
  .patch(async (req, res) => {
      ...
      if (op.op === "Replace") {
        switch (op.path) {
          ...
          case "externalId":
            ...
            break;
          case "active": {
            userUpdate.status = await getUserNewStatus(user!.id, op.value === "True", req.context);
            break;
          }
    ...
    if (isNonNullish(userUpdate.status) && user.status !== userUpdate.status) {
      if (userUpdate.status === "ACTIVE") {
        const allUsersGroups = await req.context.userGroups.loadAllUsersGroupsByOrgId(user.org_id);
        await req.context.userGroups.addUsersToGroups(
          allUsersGroups.map((ug) => ug.id),
          user.id,
          `Provisioning:${req.context.organization!.id}`,
  ...
  .put(async (req, res) => {
    ...
    if (isNonNullish(req.body.active)) {
      ...
      userUpdate.status = await getUserNewStatus(user!.id, req.body.active === "True", req.context);
    }
