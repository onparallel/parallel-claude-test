...
export const REPORTS_SERVICE = Symbol.for("REPORTS_SERVICE");

@injectable()
export class ReportsService implements IReportsService {
  constructor(@readOnly private petitions: PetitionRepository) {}

  async generateTemplateStatsReport(
    templateId: number,
    orgId: number,
    ...
    const [template, orgPetitions] = await Promise.all([
      ...
      this.petitions.getPetitionsForTemplateStatsReport(templateId, orgId, startDate, endDate),
    ]);
    ...
    const petitionEvents = (await this.petitions.getPetitionEventsByType(
