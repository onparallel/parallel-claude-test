...
export class ReportingRepository extends BaseRepository {
  ...
  async loadReportingData() {
    ...
    const petitionIds = (await this.from("petition").whereNull("deleted_at").select("id")).map(
      (r) => r.id
    );
    const petitions = (
      await pMap(
        ...
        async (batch) => {
            ...
            this.from("petition_field")
              ...
              .select(
                ...
                ...(
                  ...
                  this.knex.raw(`(count(*) filter (where type = ?))::int as ??`, [
                    type,
                    `${type.toLowerCase()}_fields`,
                  ])
                )
              ),
          ...
          return petitions.map((petition) => ({
            ...
            completed_at: petition.status === "COMPLETED" ? petition.updated_at : null,
