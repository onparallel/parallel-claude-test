...
describe("GraphQL/Petition Event Subscriptions", () => {
  ...
  describe("updatePetitionSubscription", () => {
    ...
    it("updates the subscription endpoint", async () => {
      const { errors, data } = await testClient.mutate({
        mutation: gql`
          mutation updatePetitionSubscription(
            $subscriptionId: GID!
            $newEndpoint: String!
          ) {
      ...
      expect(errors).toBeUndefined();
      expect(data!.updatePetitionSubscription).toEqual({
        id: toGlobalId("Subscription", subscriptions[0].id),
    ...
    it("sends error when trying to update a subscription shared to me with READ/WRITE", async () => {
      const { errors, data } = await testClient.mutate({
        mutation: gql`
          mutation updatePetitionSubscription(
            $subscriptionId: GID!
    ...
    it("sends error when trying to update a private subscription", async () => {
      const { errors, data } = await testClient.mutate({
        mutation: gql`
          mutation updatePetitionSubscription(
            $subscriptionId: GID!
