...
export function WithInternationalFontFamily({ children }: { children: string }) {
  const [script] = firstBy(Object.entries(RANGES), [
    ([_, test]) => children.match(test)?.length ?? 0,
    "desc",
  ])!;
  const fontFamily = ["Latin", "Cyrillic", "Greek"].includes(script)
    ? "Noto Sans"
    : `Noto Sans ${script}`;
