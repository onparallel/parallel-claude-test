...
import { ProfileFieldValueUpdatedEvent } from "../../../db/events/ProfileEvent";
...
@injectable()
export class ClientRiskUpdateListener implements EventListener<"PROFILE_FIELD_VALUE_UPDATED"> {
  public readonly types: "PROFILE_FIELD_VALUE_UPDATED"[] = ["PROFILE_FIELD_VALUE_UPDATED"];

  constructor(@inject(ProfileRepository) private profiles: ProfileRepository) {}

  public async handle(event: ProfileFieldValueUpdatedEvent) {
    for (const config of CONFIG) {
      if (
        event.org_id !== config.orgId ||
        ![
