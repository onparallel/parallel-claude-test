...
function formatDate(date: Date | null) {
  return date
    ? format(utcToZonedTime(date, "Europe/Madrid"), "yyyy-MM-dd HH:mm", {
        timeZone: "Europe/Madrid",
      })
    : "";
}

createCronWorker("reporting", async (context) => {
  const wb = new Excel.Workbook();
  const organizations = wb.addWorksheet("Organizations");
  ...
  const petitions = wb.addWorksheet("Petitions");
  petitions.columns = [
    { header: "ID", key: "id" },
    { header: "Name", key: "name" },
    { header: "Text Fields", key: "text_fields" },
    { header: "File Upload Fields", key: "file_upload_fields" },
    { header: "Creator ID", key: "owner_id" },
    { header: "Creation Date", key: "created_at" },
    { header: "Send Date", key: "sent_at" },
    {
    ...
    attachments: [
      {
        ...
        content: (await wb.xlsx.writeBuffer()) as any,
      },
    ],
  });
});
