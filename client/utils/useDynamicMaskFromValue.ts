...
export function useDynamicMaskFromValue(format: Maybe<string>, value: string) {
      ...
      IBAN: (value: string) => {
        ...
        if (value && value.length > 1) {
          ...
          const match = ibanMasks.find((options) =>
            options[0].startsWith(value.slice(0, 2).toUpperCase())
          );
  ...
  return useMemo(() => {
    return isDefined(format) ? getInputFormatMaskOptions(format, value) : {};
  }, [format, value]);
}
