...
export const ibanRegex = (() => {
  return new RegExp(
    [
      ...
      ibanDefinitions
        .map(([country, format]) => {
          return `(${country}\\d{2}${format
            .map(([pattern, repeat]) => {
              return { A: "[\\dA-Z]", F: "\\d", U: "[A-Z]" }[pattern] + `{${repeat}}`;
            })
            .join("")})`;
        })
        .join("|"),
