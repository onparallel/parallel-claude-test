...
export const PetitionComposeFieldList = memo(function PetitionComposeFieldList({
  petition,
  activeFieldId,
  showErrors,
      ...
      >
        {fieldsWithIndices.map(([field, fieldIndex, childrenFieldIndices], i) => {
          ...
          const showAddFieldButton =
            [field.id, nextFieldId].some((id) => id === hoveredFieldId || id === focusedFieldId) &&
          ...
          return (
            <Fragment key={field.id}>
              <PetitionComposeField
                ...
                activeChildFieldId={
                  isNonNullish(field.children) && field.children.some((f) => f.id === activeFieldId)
                    ? activeFieldId
                    : null
                }
