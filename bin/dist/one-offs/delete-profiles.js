...
async function main() {
    ...
    do {
        ...
        const result = await (0, apiHelpers_1.apiRequest)("/profiles", {
        ...
        for (const item of result.items) {
            ...
            await (0, apiHelpers_1.apiRequest)(`/profiles/${item.id}/close`, { method: "POST" });
            console.log(`Deleting profile ${item.id}`);
            await (0, apiHelpers_1.apiRequest)(`/profiles/${item.id}`, { method: "DELETE" });
        }
    } while (totalCount > 0);
}
(0, run_1.run)(main);
