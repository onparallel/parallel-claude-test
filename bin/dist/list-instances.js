...
async function main() {
    ...
    table.push(...instances
        .filter((i) => i.State?.Name !== "terminated" && (includeStopped || i.State?.Name !== "stopped"))
        .map((i) => {
        const state = (() => {
            switch (i.State?.Name) {
                case "running":
                ...
                default:
                    return chalk_1.default.yellow(i.State?.Name);
            }
        })();
        const health = (() => {
        ...
        return [
            i.InstanceId,
            i.PrivateIpAddress,
            i.Tags?.find((t) => t.Key === "Name")?.Value ?? chalk_1.default.gray `-`,
            i.Tags?.find((t) => t.Key === "Release")?.Value ?? chalk_1.default.gray `-`,
            i.Placement?.AvailabilityZone,
            state,
            instancesToLb[i.InstanceId] ?? chalk_1.default.red `⨯`,
            health,
